...
| PUT | `/broadcast` | Uploads .wav chunk to be transcribed inside existing real-time session| access_token, RTSessionID, Word Detection (True/False), UploadFile | JSON Object |
| PUT | `/upload` | Uploads .wav audio file to be transcribed | access_token, Word Detection (True/False), UploadFile | JSON Object |
| POST | `/register` | Creates new user | AdminUsername, AdminPasswords, Username, Password | JSON Object |
| POST | `/login` | User login | Username, Password | String "access_token" |
| POST | `/logout` | User logout | access_token | String "logout completed" |
...
| POST | `/deleteIWordsCsv` | Removes key terms (or all of them) from the dictionary | AdminUsername, AdminPasswords, DeleteAll ( 1 or 0 ) | JSON Object |
